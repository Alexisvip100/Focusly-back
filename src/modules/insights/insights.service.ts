...
export class InsightsService {
    ...
    async getInsights(userId: string, filter: string): Promise<InsightsResponse> {
        ...
        const filteredTasks = allTasks.filter((t) => {
            const taskDate = new Date(t.createdAt); // Or updatedAt, or completedAt depending on metric
        ...
        // Total Focus Hours
        const totalMinutes = filteredTasks.reduce((acc, t) => acc + (t.realTimer || 0), 0);
        ...
        const completedInPeriod = filteredTasks.filter((t) => t.status === TaskStatus.Done).length;
        ...
        // Golden Window (Mocked logic or simple calculation)
        // Real logic: Look at completedAt hours
        const completedTasks = allTasks.filter(t => t.status === TaskStatus.Done && t.completedAt);
        // Simple bucket
        ...
        const colors = ['#3b82f6', '#6366f1', '#8b5cf6', '#64748b', '#ef4444', '#10b981'];
        let colorIdx = 0;
